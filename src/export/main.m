...
static NSURL *state_file_url(void)
{
    ...
    if (!identifier) identifier = @MORPHEUS_EXPORT_BUNDLE_ID;
...
static void restore_state(const morph_app_api *api, morph_host *host, void **state)
{
    NSData *data = [NSData dataWithContentsOfURL:state_file_url()];
...
static void save_state(const morph_app_api *api, morph_host *host, void *state)
{
...
int main(void)
{
    ...
    window = SDL_CreateWindow(api->name ? api->name : MORPHEUS_EXPORT_APP_NAME,
